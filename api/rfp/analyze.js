...
const extractWithGroq = async (rawText) => {
  const rfpText = rawText.slice(0, 8000); // Increased context window

  const systemPrompt = `You are a Lead Procurement Intelligence Agent. Your mission is to decompose a complex RFP document into atomic, actionable requirements.
Follow these principles:
1. Atomicity: Each requirement must be a single deliverable or constraint.
2. Precision: Isolate exact deadlines, budget limits, and technical specifications.
3. Priority: Distinguish between Mandatory (Must) and Desirable (Should) items.
4. Categorization: Identify Evaluation Criteria (how we are scored) vs Requirements (what we do).`;

  const extractionPrompt = `Analyze the following RFP text and return a structured JSON mission profile.
  
Rules:
- JSON structure only.
- No markdown outside of the JSON.
- Requirements should be concise (max 160 chars).
- Do not hallucinate requirements not present in text.
