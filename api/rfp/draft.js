...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "POST") {
      ...
      const { generateAgenticDraft } = await import("../../bid-engine/lib/agents/coordinator.js");

      // Use the Agentic Coordinator to generate high-quality drafts
      // We run them in parallel but limit to the first 5 to avoid timeouts/rate-limits
      const draftingPool = targets.slice(0, 5);
      const draftsList = await Promise.all(draftingPool.map(async (target) => {
        try {
          return await generateAgenticDraft(target, capabilitySource, {
            tone: body.tone || "expert and highly technical",
            entities: extractedEntities
          });
        } catch (err) {
          console.error(`Agentic drafting failed for requirement ${target.id}:`, err);
          // Fallback to basic draft if agent fails
          const match = matchRequirementToCapabilities(target, capabilitySource, { entities: extractedEntities });
          const capability = match.capability || capabilitySource[0];
