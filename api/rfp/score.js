...
export default async function handler(req, res) {
  try {
    ...
    const { consultOnBid } = await import("../../bid-engine/lib/agents/consultant.js");

    // Supplement numerical scores with Strategic Agent Consultation
    let consultation = null;
    try {
      consultation = await consultOnBid(scores, requirements, bidHistory);
    } catch (err) {
    ...
    if (workspaceId && isUuid(workspaceId)) {
      const { data: savedScore, error: dbError } = await workspaceDb
        ...
        .upsert({
          ...
          decision: consultation?.recommendation || scores.decision,
          // We can't store the full JSON in the current schema without an update,
          // but we can store the decision.
    ...
    return res.status(200).json({
      ...
      workspaceId,
      scores,
      consultation, // Return the agent's insights to the frontend
      record: record || {
        total_score: scores.total_score,
        budget_alignment: scores.budget_alignment,
        capability_match: scores.capability_match,
